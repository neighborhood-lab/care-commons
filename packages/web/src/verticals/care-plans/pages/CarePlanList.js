...
const CarePlanList = () => {
    ...
    const { data, isLoading, error, refetch } = (0, hooks_2.useCarePlans)(filters);
    if (isLoading) {
    ...
    if (error) {
        return (<components_1.ErrorMessage message={error.message || 'Failed to load care plans'} retry={refetch}/>);
    }
    ...
    return (<div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          {can('care_plans:write') && (<react_router_dom_1.Link to="/care-plans/new">
      ...
      <components_2.CarePlanSearch filters={filters} onFiltersChange={setFilters}/>

      {carePlans.length === 0 ? (<components_1.EmptyState title="No care plans found" description="Get started by creating your first care plan." action={can('care_plans:write') ? (<react_router_dom_1.Link to="/care-plans/new">
                <components_1.Button leftIcon={<lucide_react_1.Plus className="h-4 w-4"/>}>
                  Create Care Plan
                </components_1.Button>
                ...
                : 'space-y-4'}>
          {carePlans.map((carePlan) => (<components_2.CarePlanCard key={carePlan.id} carePlan={carePlan} compact={viewMode === 'list'}/>))}
        </div>)}
...
exports.CarePlanList = CarePlanList;
//# sourceMappingURL=CarePlanList.js.map