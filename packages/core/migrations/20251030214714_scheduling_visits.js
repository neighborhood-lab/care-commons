...
async function up(knex) {
    await knex.schema.createTable('service_patterns', (table) => {
        ...
        table.check(`preferred_time_of_day IN ('EARLY_MORNING', 'MORNING', 'AFTERNOON', 'EVENING', 'NIGHT', 'ANY')`);
        ...
        table.check(`authorization_end_date IS NULL OR authorization_start_date IS NULL OR authorization_start_date <= authorization_end_date`);
    ...
    await knex.raw('CREATE INDEX idx_patterns_organization ON service_patterns(organization_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_patterns_branch ON service_patterns(branch_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_patterns_client ON service_patterns(client_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_patterns_status ON service_patterns(status) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_patterns_active ON service_patterns(client_id, status) WHERE deleted_at IS NULL AND status = \'ACTIVE\'');
    await knex.raw('CREATE INDEX idx_patterns_dates ON service_patterns(effective_from, effective_to) WHERE deleted_at IS NULL');
    await knex.raw(`
  ...
  `);
    ...
    await knex.schema.createTable('visits', (table) => {
        ...
        table.timestamp('deleted_at');
        table.uuid('deleted_by');
        table.check(`visit_type IN ('REGULAR', 'INITIAL', 'DISCHARGE', 'RESPITE', 'EMERGENCY', 'MAKEUP', 'SUPERVISION', 'ASSESSMENT')`);
        table.check(`scheduled_duration BETWEEN 15 AND 1440`);
        table.check(`assignment_method IN ('MANUAL', 'AUTO_MATCH', 'SELF_ASSIGN', 'PREFERRED', 'OVERFLOW')`);
        table.check(`status IN ('DRAFT', 'SCHEDULED', 'UNASSIGNED', 'ASSIGNED', 'CONFIRMED', 'EN_ROUTE', 'ARRIVED', 'IN_PROGRESS', 'PAUSED', 'COMPLETED', 'INCOMPLETE', 'CANCELLED', 'NO_SHOW_CLIENT', 'NO_SHOW_CAREGIVER', 'REJECTED')`);
        ...
        table.check(`actual_end_time IS NULL OR actual_start_time IS NULL OR actual_start_time < actual_end_time`);
        table.foreign('organization_id').references('id').inTable('organizations');
        table.foreign('branch_id').references('id').inTable('branches');
        table.foreign('client_id').references('id').inTable('clients');
        table.foreign('pattern_id').references('id').inTable('service_patterns');
    ...
    await knex.raw('CREATE INDEX idx_visits_organization ON visits(organization_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_branch ON visits(branch_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_client ON visits(client_id, scheduled_date) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_caregiver ON visits(assigned_caregiver_id, scheduled_date) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_pattern ON visits(pattern_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_schedule ON visits(schedule_id) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_date ON visits(scheduled_date, scheduled_start_time) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_status ON visits(status) WHERE deleted_at IS NULL');
    await knex.raw('CREATE INDEX idx_visits_billing ON visits(billing_status) WHERE deleted_at IS NULL AND billing_status IS NOT NULL');
    await knex.raw(`
  ...
  `);
    await knex.schema.createTable('visit_exceptions', (table) => {
        ...
        table.timestamp('created_at').notNullable().defaultTo(knex.fn.now());
        table.timestamp('updated_at').notNullable().defaultTo(knex.fn.now());
        table.check(`exception_type IN ('LATE_START', 'EARLY_END', 'OVERTIME', 'NO_SHOW_CLIENT', 'NO_SHOW_CAREGIVER', 'LOCATION_MISMATCH', 'MISSED_TASKS', 'SAFETY_CONCERN', 'EQUIPMENT_ISSUE', 'MEDICATION_ISSUE', 'CLIENT_REFUSED', 'EMERGENCY', 'OTHER')`);
    ...
    await knex.raw('CREATE INDEX idx_exceptions_severity ON visit_exceptions(severity) WHERE status != \'RESOLVED\'');
    await knex.raw('CREATE INDEX idx_exceptions_followup ON visit_exceptions(followup_assigned_to) WHERE requires_followup = true AND status != \'RESOLVED\'');
    await knex.raw("COMMENT ON TABLE service_patterns IS 'Templates for recurring service schedules'");
    ...
    await knex.raw("COMMENT ON COLUMN service_patterns.recurrence IS 'JSONB: Recurrence rule with frequency, interval, days/dates, start/end times, timezone'");
    await knex.raw("COMMENT ON COLUMN service_patterns.flexibility_window IS 'Allowed variance in minutes from scheduled time'");
    await knex.raw("COMMENT ON COLUMN visits.status_history IS 'JSONB: Array of status change records with timestamps and reasons'");
    await knex.raw("COMMENT ON COLUMN visits.location_verification IS 'JSONB: GPS coordinates and verification data for EVV compliance'");
    await knex.raw("COMMENT ON COLUMN visits.verification_data IS 'JSONB: Clock in/out verification records'");
    await knex.raw("COMMENT ON COLUMN visits.signature_data IS 'JSONB: Digital signature capture data'");
}
...
//# sourceMappingURL=20251030214714_scheduling_visits.js.map