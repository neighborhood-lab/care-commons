...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('care_plans', (table) => {
    ...
    // Constraints
    table.check(
      `plan_type IN ('PERSONAL_CARE', 'COMPANION', 'SKILLED_NURSING', 'THERAPY', 'HOSPICE', 'RESPITE', 'LIVE_IN', 'CUSTOM')`
    );
    table.check(
      `status IN ('DRAFT', 'PENDING_APPROVAL', 'ACTIVE', 'ON_HOLD', 'EXPIRED', 'DISCONTINUED', 'COMPLETED')`
    );
  ...
  await knex.schema.createTable('task_instances', (table) => {
    ...
    table.uuid('template_id'); // Reference to template within care plan (stored in JSONB)
    table.uuid('visit_id'); // Will reference visits table when created
    table.uuid('client_id').notNullable();
    ...
    // Constraints
    table.check(
      `category IN ('PERSONAL_HYGIENE', 'BATHING', 'DRESSING', 'GROOMING', 'TOILETING', 'MOBILITY', 'TRANSFERRING', 'AMBULATION', 'MEDICATION', 'MEAL_PREPARATION', 'FEEDING', 'HOUSEKEEPING', 'LAUNDRY', 'SHOPPING', 'TRANSPORTATION', 'COMPANIONSHIP', 'MONITORING', 'DOCUMENTATION', 'OTHER')`
    );
    table.check(
      `time_of_day IN ('EARLY_MORNING', 'MORNING', 'AFTERNOON', 'EVENING', 'NIGHT', 'OVERNIGHT', 'ANY')`
    );
    table.check(
      `status IN ('SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'SKIPPED', 'MISSED', 'CANCELLED', 'ISSUE_REPORTED')`
    );
  ...
  await knex.schema.createTable('progress_notes', (table) => {
    ...
    // Constraints
    table.check(
      `note_type IN ('VISIT_NOTE', 'WEEKLY_SUMMARY', 'MONTHLY_SUMMARY', 'CARE_PLAN_REVIEW', 'INCIDENT', 'CHANGE_IN_CONDITION', 'COMMUNICATION', 'OTHER')`
    );
  ...
  await knex.raw(
    "COMMENT ON COLUMN task_instances.quality_check_responses IS 'Responses to quality check questions for task'"
  );
  await knex.raw(
