...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('payers', (table) => {
    ...
    table.jsonb('submission_methods'); // Array of submission methods
    ...
    table.decimal('denial_rate', 5, 2); // Percentage

    // Metadata
    table.text('notes');
    table.jsonb('contacts'); // Array of payer contacts
  ...
  `);
  await knex.raw(
    'ALTER TABLE payers ADD CONSTRAINT chk_payment_terms_days CHECK (payment_terms_days BETWEEN 0 AND 365)'
  );
  await knex.raw(
    'ALTER TABLE payers ADD CONSTRAINT chk_claim_filing_limit CHECK (claim_filing_limit IS NULL OR claim_filing_limit BETWEEN 0 AND 365)'
  );
  await knex.raw(`
  ...
  `);
  await knex.raw(
    'ALTER TABLE rate_schedules ADD CONSTRAINT chk_rate_schedule_dates CHECK (effective_to IS NULL OR effective_from <= effective_to)'
  );
  await knex.raw(`
  ...
  await knex.raw(
    "CREATE INDEX idx_rate_schedules_active ON rate_schedules(organization_id, status) WHERE status = 'ACTIVE'"
  );
  ...
  await knex.schema.createTable('service_authorizations', (table) => {
    ...
    table.jsonb('allowed_providers'); // Array of caregiver IDs
    ...
    // Documents
    table.jsonb('document_ids'); // Array of document IDs
  ...
  `);
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_authorized_units CHECK (authorized_units >= 0)'
  );
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_used_units CHECK (used_units >= 0)'
  );
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_remaining_units CHECK (remaining_units >= 0)'
  );
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_billed_units CHECK (billed_units >= 0)'
  );
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_authorization_dates CHECK (effective_from <= effective_to)'
  );
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_authorization_units CHECK (used_units <= authorized_units)'
  );
  await knex.raw(
    'ALTER TABLE service_authorizations ADD CONSTRAINT chk_expiration_warning_days CHECK (expiration_warning_days IS NULL OR expiration_warning_days BETWEEN 0 AND 90)'
  );
  await knex.raw(`
  ...
  await knex.raw(
    'CREATE UNIQUE INDEX idx_authorizations_number ON service_authorizations(authorization_number) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_authorizations_organization ON service_authorizations(organization_id) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_authorizations_client ON service_authorizations(client_id, status) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_authorizations_payer ON service_authorizations(payer_id, status) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_authorizations_status ON service_authorizations(status, effective_to) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    "CREATE INDEX idx_authorizations_expiring ON service_authorizations(effective_to) WHERE status = 'ACTIVE' AND deleted_at IS NULL"
  );
  await knex.raw(
    "CREATE INDEX idx_authorizations_low_units ON service_authorizations(remaining_units) WHERE status = 'ACTIVE' AND remaining_units < 10 AND deleted_at IS NULL"
  );
  ...
  await knex.schema.createTable('billable_items', (table) => {
    ...
    // Modifiers
    table.jsonb('modifiers'); // Array of billing modifiers
    table.jsonb('adjustments'); // Array of adjustments
    ...
    table
      .foreign('rate_schedule_id')
      .references('id')
      .inTable('rate_schedules')
      .onDelete('SET NULL');
    table
      .foreign('authorization_id')
      .references('id')
      .inTable('service_authorizations')
      .onDelete('SET NULL');
    table.foreign('payer_id').references('id').inTable('payers').onDelete('RESTRICT');
  });
  ...
  // Add complex constraints
  await knex.raw(
    'ALTER TABLE billable_items ADD CONSTRAINT chk_duration_minutes CHECK (duration_minutes >= 0)'
  );
  ...
  await knex.raw(
    'ALTER TABLE billable_items ADD CONSTRAINT chk_final_amount CHECK (final_amount >= 0)'
  );
  await knex.raw(
    'ALTER TABLE billable_items ADD CONSTRAINT chk_authorization_remaining_units CHECK (authorization_remaining_units IS NULL OR authorization_remaining_units >= 0)'
  );
  await knex.raw(`
  ...
  `);
  await knex.raw(
    'ALTER TABLE billable_items ADD CONSTRAINT chk_paid_amount CHECK (paid_amount IS NULL OR paid_amount >= 0)'
  );
  await knex.raw(`
  ...
  await knex.raw(
    'CREATE INDEX idx_billable_organization ON billable_items(organization_id, service_date) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_branch ON billable_items(branch_id, service_date) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_client ON billable_items(client_id, service_date) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_caregiver ON billable_items(caregiver_id, service_date) WHERE caregiver_id IS NOT NULL AND deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_payer ON billable_items(payer_id, service_date) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_status ON billable_items(status, service_date) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_visit ON billable_items(visit_id) WHERE visit_id IS NOT NULL AND deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_evv ON billable_items(evv_record_id) WHERE evv_record_id IS NOT NULL AND deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_invoice ON billable_items(invoice_id) WHERE invoice_id IS NOT NULL AND deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_authorization ON billable_items(authorization_id) WHERE authorization_id IS NOT NULL AND deleted_at IS NULL'
  );
  await knex.raw(
    "CREATE INDEX idx_billable_ready ON billable_items(organization_id, payer_id) WHERE status = 'READY' AND deleted_at IS NULL"
  );
  await knex.raw(
    'CREATE INDEX idx_billable_hold ON billable_items(organization_id) WHERE is_hold = true AND deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_billable_review ON billable_items(organization_id) WHERE requires_review = true AND deleted_at IS NULL'
  );
  await knex.raw(
    "CREATE INDEX idx_billable_unpaid ON billable_items(payer_id, service_date) WHERE is_paid = false AND status NOT IN ('VOIDED', 'HOLD') AND deleted_at IS NULL"
  );
  ...
  await knex.schema.createTable('invoices', (table) => {
    ...
    table.jsonb('billable_item_ids').notNullable(); // Array of billable item IDs
    table.jsonb('line_items').notNullable(); // Array of invoice line items
    ...
    table.jsonb('payments').notNullable().defaultTo('[]'); // Array of payment references
    ...
    table.jsonb('claim_ids'); // Array of claim IDs
  ...
  await knex.raw(
    'ALTER TABLE invoices ADD CONSTRAINT chk_discount_amount CHECK (discount_amount >= 0)'
  );
  await knex.raw(
    'ALTER TABLE invoices ADD CONSTRAINT chk_adjustment_amount CHECK (adjustment_amount >= 0)'
  );
  ...
  `);
  await knex.raw(
    'ALTER TABLE invoices ADD CONSTRAINT chk_invoice_period CHECK (period_start <= period_end)'
  );
  await knex.raw(
    'ALTER TABLE invoices ADD CONSTRAINT chk_invoice_dates CHECK (invoice_date <= due_date)'
  );
  await knex.raw(
    'ALTER TABLE invoices ADD CONSTRAINT chk_invoice_balance CHECK (balance_due = total_amount - paid_amount)'
  );
  ...
  await knex.raw(
    'CREATE INDEX idx_invoices_organization ON invoices(organization_id, invoice_date DESC) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_invoices_branch ON invoices(branch_id, invoice_date DESC) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_invoices_payer ON invoices(payer_id, invoice_date DESC) WHERE deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_invoices_client ON invoices(client_id, invoice_date DESC) WHERE client_id IS NOT NULL AND deleted_at IS NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_invoices_status ON invoices(status, due_date) WHERE deleted_at IS NULL'
  );
  await knex.raw(
  ...
  await knex.schema.createTable('payments', (table) => {
    ...
    // Application
    table.jsonb('allocations').notNullable().defaultTo('[]'); // Array of payment allocations
  ...
  await knex.raw(
    'ALTER TABLE payments ADD CONSTRAINT chk_unapplied_amount CHECK (unapplied_amount >= 0)'
  );
  await knex.raw(`
  ...
  `);
  await knex.raw(
    'ALTER TABLE payments ADD CONSTRAINT chk_payment_dates CHECK (received_date >= payment_date)'
  );
  ...
  await knex.raw(
    'CREATE INDEX idx_payments_reference ON payments(reference_number) WHERE reference_number IS NOT NULL'
  );
  await knex.raw(
    'CREATE INDEX idx_payments_unreconciled ON payments(organization_id) WHERE is_reconciled = false'
  );
  await knex.raw(
  ...
  await knex.schema.createTable('claims', (table) => {
    ...
    // Billable items
    table.jsonb('billable_item_ids').notNullable(); // Array of billable item IDs
    table.jsonb('line_items').notNullable(); // Array of claim line items
    ...
    table.string('control_number', 100); // Payer-assigned
    ...
    table.jsonb('supporting_document_ids'); // Array of document IDs
  ...
  await knex.raw(
    'ALTER TABLE claims ADD CONSTRAINT chk_total_approved CHECK (total_approved IS NULL OR total_approved >= 0)'
  );
  await knex.raw(
    'ALTER TABLE claims ADD CONSTRAINT chk_total_paid CHECK (total_paid IS NULL OR total_paid >= 0)'
  );
  await knex.raw(
    'ALTER TABLE claims ADD CONSTRAINT chk_total_adjustments CHECK (total_adjustments IS NULL OR total_adjustments >= 0)'
  );
  await knex.raw(
    'ALTER TABLE claims ADD CONSTRAINT chk_patient_responsibility CHECK (patient_responsibility IS NULL OR patient_responsibility >= 0)'
  );
  await knex.raw(`
  ...
  await knex.raw(
    "CREATE INDEX idx_claims_denied ON claims(organization_id) WHERE status = 'DENIED' AND is_appealable = true"
  );
  await knex.raw(`
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw(
    'DROP TRIGGER IF EXISTS trigger_authorizations_updated_at ON service_authorizations'
  );
