...
async function up(knex) {
    await knex.schema.createTable('evv_records', (table) => {
        ...
        table.jsonb('mid_visit_checks');
        table.jsonb('pause_events');
        table.jsonb('exception_events');
        ...
        table.string('integrity_checksum', 64).notNullable();
        ...
        table.check(`record_status IN ('PENDING', 'COMPLETE', 'SUBMITTED', 'APPROVED', 'REJECTED', 'DISPUTED', 'AMENDED', 'VOIDED')`);
        ...
        table.check(`payor_approval_status IN ('PENDING', 'APPROVED', 'DENIED', 'PENDING_INFO', 'APPEALED')`);
    ...
    await knex.raw('CREATE INDEX idx_evv_submission ON evv_records(submitted_to_payor) WHERE submitted_to_payor IS NOT NULL');
    await knex.raw('CREATE INDEX idx_evv_pending ON evv_records(organization_id, service_date) WHERE record_status = \'PENDING\'');
    await knex.raw(`
  ...
  `);
    ...
    await knex.raw('CREATE INDEX idx_time_entries_organization ON time_entries(organization_id, entry_timestamp)');
    await knex.raw('CREATE INDEX idx_time_entries_caregiver ON time_entries(caregiver_id, entry_timestamp)');
    await knex.raw('CREATE INDEX idx_time_entries_client ON time_entries(client_id, entry_timestamp)');
    ...
    await knex.raw('CREATE INDEX idx_time_entries_device ON time_entries(device_id, entry_timestamp)');
    await knex.raw('CREATE INDEX idx_time_entries_offline ON time_entries(offline_recorded, status) WHERE offline_recorded = true');
    await knex.raw('CREATE INDEX idx_time_entries_pending ON time_entries(organization_id, status) WHERE status = \'PENDING\'');
    await knex.raw('CREATE INDEX idx_time_entries_flagged ON time_entries(organization_id, entry_timestamp) WHERE status = \'FLAGGED\'');
    await knex.raw(`
  ...
  `);
    await knex.schema.createTable('geofences', (table) => {
        ...
        table.jsonb('polygon_points');
        table.boolean('is_active').defaultTo(true);
        table.integer('allowed_variance').defaultTo(0);
        ...
        table.decimal('average_accuracy', 8, 2);
    ...
    await knex.raw('CREATE INDEX idx_geofences_address ON geofences(address_id) WHERE is_active = true AND status = \'ACTIVE\'');
    await knex.raw('CREATE INDEX idx_geofences_location ON geofences(center_latitude, center_longitude) WHERE is_active = true AND status = \'ACTIVE\'');
    await knex.raw('CREATE INDEX idx_geofences_active ON geofences(organization_id, status) WHERE is_active = true AND status = \'ACTIVE\'');
    await knex.raw('CREATE INDEX idx_geofences_performance ON geofences(verification_count DESC, average_accuracy) WHERE is_active = true');
    await knex.raw(`
  ...
  `);
    await knex.raw("COMMENT ON TABLE evv_records IS 'Electronic Visit Verification records for compliance and billing'");
    await knex.raw("COMMENT ON TABLE time_entries IS 'Individual clock-in/out events with location verification'");
    await knex.raw("COMMENT ON TABLE geofences IS 'Virtual boundaries for location verification'");
    await knex.raw("COMMENT ON COLUMN evv_records.service_type_code IS 'Federal requirement: Type of service performed'");
    await knex.raw("COMMENT ON COLUMN evv_records.client_name IS 'Federal requirement: Individual receiving service (encrypted)'");
    await knex.raw("COMMENT ON COLUMN evv_records.caregiver_name IS 'Federal requirement: Individual providing service'");
    await knex.raw("COMMENT ON COLUMN evv_records.service_date IS 'Federal requirement: Date of service'");
    await knex.raw("COMMENT ON COLUMN evv_records.service_address IS 'Federal requirement: Location of service delivery'");
    await knex.raw("COMMENT ON COLUMN evv_records.clock_in_time IS 'Federal requirement: Time service begins'");
    await knex.raw("COMMENT ON COLUMN evv_records.clock_out_time IS 'Federal requirement: Time service ends'");
    await knex.raw("COMMENT ON COLUMN evv_records.clock_in_verification IS 'JSONB: GPS coordinates, accuracy, device info for clock-in'");
    await knex.raw("COMMENT ON COLUMN evv_records.clock_out_verification IS 'JSONB: GPS coordinates, accuracy, device info for clock-out'");
    await knex.raw("COMMENT ON COLUMN evv_records.compliance_flags IS 'JSONB: Array of compliance flags (COMPLIANT, GEOFENCE_VIOLATION, etc.)'");
    await knex.raw("COMMENT ON COLUMN evv_records.integrity_hash IS 'SHA-256 hash of core EVV data for tamper detection'");
    await knex.raw("COMMENT ON COLUMN evv_records.sync_metadata IS 'JSONB: Sync status, IDs, timestamps for offline capability'");
    await knex.raw("COMMENT ON COLUMN time_entries.location IS 'JSONB: Complete location verification data'");
    await knex.raw("COMMENT ON COLUMN time_entries.device_info IS 'JSONB: Device model, OS, app version, security status'");
    await knex.raw("COMMENT ON COLUMN time_entries.integrity_hash IS 'SHA-256 hash for tamper detection'");
    await knex.raw("COMMENT ON COLUMN time_entries.sync_metadata IS 'JSONB: Offline sync status and conflict resolution'");
    await knex.raw("COMMENT ON COLUMN time_entries.manual_override IS 'JSONB: Supervisor override details if verification failed'");
    await knex.raw("COMMENT ON COLUMN geofences.radius_meters IS 'Geofence radius in meters (10-500m allowed)'");
    await knex.raw("COMMENT ON COLUMN geofences.allowed_variance IS 'Additional tolerance in meters for GPS accuracy'");
    await knex.raw("COMMENT ON COLUMN geofences.verification_count IS 'Total verification attempts at this location'");
    await knex.raw("COMMENT ON COLUMN geofences.successful_verifications IS 'Count of successful verifications'");
    await knex.raw("COMMENT ON COLUMN geofences.failed_verifications IS 'Count of failed verifications'");
    await knex.raw("COMMENT ON COLUMN geofences.average_accuracy IS 'Running average of GPS accuracy at this location'");
}
...
//# sourceMappingURL=20251030214715_evv_tables.js.map