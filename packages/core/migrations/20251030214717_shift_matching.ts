...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('open_shifts', (table) => {
    ...
    table
      ...
      .notNullable()
      .defaultTo('NORMAL')
      .checkIn(['LOW', 'NORMAL', 'HIGH', 'CRITICAL']);
    table.boolean('is_urgent').defaultTo(false);
    table.timestamp('fill_by_date');

    // Matching metadata
    table
      .string('matching_status', 50)
      .notNullable()
      .defaultTo('NEW')
      .checkIn(['NEW', 'MATCHING', 'MATCHED', 'PROPOSED', 'ASSIGNED', 'NO_MATCH', 'EXPIRED']);
  ...
  await knex.raw(
    'ALTER TABLE open_shifts ADD CONSTRAINT chk_fill_by_future CHECK (fill_by_date IS NULL OR fill_by_date > NOW())'
  );
  ...
  await knex.schema.createTable('matching_configurations', (table) => {
    ...
    table
      .string('optimize_for', 50)
      .notNullable()
      .defaultTo('BEST_MATCH')
      .checkIn([
        'BEST_MATCH',
        'FASTEST_FILL',
        'COST_EFFICIENT',
  ...
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT chk_max_travel_distance CHECK (max_travel_distance IS NULL OR max_travel_distance > 0)'
  );
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT chk_max_travel_time CHECK (max_travel_time IS NULL OR max_travel_time > 0)'
  );
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT chk_auto_assign_threshold CHECK (auto_assign_threshold IS NULL OR (auto_assign_threshold >= 0 AND auto_assign_threshold <= 100))'
  );
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT chk_min_score_for_proposal CHECK (min_score_for_proposal >= 0 AND min_score_for_proposal <= 100)'
  );
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT chk_max_proposals_per_shift CHECK (max_proposals_per_shift >= 1 AND max_proposals_per_shift <= 20)'
  );
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT chk_proposal_expiration_minutes CHECK (proposal_expiration_minutes >= 15 AND proposal_expiration_minutes <= 1440)'
  );
  await knex.raw(
    'ALTER TABLE matching_configurations ADD CONSTRAINT unique_default_per_scope UNIQUE NULLS NOT DISTINCT (organization_id, branch_id, is_default) DEFERRABLE INITIALLY DEFERRED'
  );
  ...
  await knex.schema.createTable('assignment_proposals', (table) => {
    ...
    table
      .string('match_quality', 50)
      .notNullable()
      .checkIn(['EXCELLENT', 'GOOD', 'FAIR', 'POOR', 'INELIGIBLE']);
    table.jsonb('match_reasons').notNullable().defaultTo('[]');

    // Proposal lifecycle
    table
      .string('proposal_status', 50)
      .notNullable()
      .defaultTo('PENDING')
      .checkIn([
        'PENDING',
    ...
    table
      .string('proposal_method', 50)
      .notNullable()
      .checkIn(['AUTOMATIC', 'MANUAL', 'CAREGIVER_SELF_SELECT']);

    // Response tracking
    table.boolean('sent_to_caregiver').defaultTo(false);
    table.timestamp('sent_at');
    table
      .string('notification_method', 50)
      .checkIn(['PUSH', 'SMS', 'EMAIL', 'PHONE_CALL', 'IN_APP']);
  ...
  // Add complex constraints
  await knex.raw(
    'ALTER TABLE assignment_proposals ADD CONSTRAINT chk_match_score CHECK (match_score >= 0 AND match_score <= 100)'
  );
  await knex.raw(`
  ...
  await knex.raw(
    "CREATE INDEX idx_proposals_pending ON assignment_proposals(caregiver_id, proposed_at) WHERE deleted_at IS NULL AND proposal_status IN ('PENDING', 'SENT', 'VIEWED')"
  );
  await knex.raw(
    "CREATE INDEX idx_proposals_expiring ON assignment_proposals(sent_at) WHERE deleted_at IS NULL AND proposal_status IN ('SENT', 'VIEWED')"
  );
  await knex.raw(
  ...
  await knex.schema.createTable('caregiver_preference_profiles', (table) => {
    ...
    table
      ...
      .notNullable()
      .defaultTo(JSON.stringify(['PUSH', 'SMS']));
  ...
  // Add complex constraints
  await knex.raw(
    'ALTER TABLE caregiver_preference_profiles ADD CONSTRAINT chk_max_travel_distance CHECK (max_travel_distance IS NULL OR max_travel_distance > 0)'
  );
  await knex.raw(
    'ALTER TABLE caregiver_preference_profiles ADD CONSTRAINT chk_max_shifts_per_day CHECK (max_shifts_per_day IS NULL OR (max_shifts_per_day >= 1 AND max_shifts_per_day <= 10))'
  );
  await knex.raw(
    'ALTER TABLE caregiver_preference_profiles ADD CONSTRAINT chk_max_shifts_per_week CHECK (max_shifts_per_week IS NULL OR (max_shifts_per_week >= 1 AND max_shifts_per_week <= 50))'
  );
  await knex.raw(
    'ALTER TABLE caregiver_preference_profiles ADD CONSTRAINT chk_max_hours_per_week CHECK (max_hours_per_week IS NULL OR (max_hours_per_week >= 1 AND max_hours_per_week <= 168))'
  );
  await knex.raw(
    'ALTER TABLE caregiver_preference_profiles ADD CONSTRAINT chk_minimum_hours_between_shifts CHECK (require_minimum_hours_between_shifts IS NULL OR require_minimum_hours_between_shifts >= 0)'
  );
  await knex.raw(
    'ALTER TABLE caregiver_preference_profiles ADD CONSTRAINT chk_quiet_hours CHECK (quiet_hours_end IS NULL OR quiet_hours_start IS NULL OR quiet_hours_start != quiet_hours_end)'
  );
  ...
  await knex.schema.createTable('bulk_match_requests', (table) => {
    ...
    table
      ...
      .notNullable()
      .defaultTo('PENDING')
      .checkIn(['PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'CANCELLED']);
  ...
  await knex.raw(
    'ALTER TABLE bulk_match_requests ADD CONSTRAINT chk_bulk_dates CHECK (date_from <= date_to)'
  );
  ...
  await knex.schema.createTable('match_history', (table) => {
    ...
    table.foreign('open_shift_id').references('id').inTable('open_shifts').onDelete('CASCADE');
    table
      .foreign('assignment_proposal_id')
      .references('id')
      .inTable('assignment_proposals')
      .onDelete('SET NULL');
  });

  // Add complex constraints
  await knex.raw(
    'ALTER TABLE match_history ADD CONSTRAINT chk_attempt_number CHECK (attempt_number > 0)'
  );
  await knex.raw(
    'ALTER TABLE match_history ADD CONSTRAINT chk_match_score CHECK (match_score IS NULL OR (match_score >= 0 AND match_score <= 100))'
  );
  await knex.raw(
    'ALTER TABLE match_history ADD CONSTRAINT chk_response_time_minutes CHECK (response_time_minutes IS NULL OR response_time_minutes >= 0)'
  );
  ...
  await knex.raw(
    "CREATE INDEX idx_history_performance ON match_history(caregiver_id, outcome, response_time_minutes) WHERE caregiver_id IS NOT NULL AND outcome IN ('ACCEPTED', 'REJECTED')"
  );
