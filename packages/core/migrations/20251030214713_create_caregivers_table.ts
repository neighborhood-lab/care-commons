...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('caregivers', (table) => {
    ...
    table.unique('email');
    table.check(
      `employment_type IN ('FULL_TIME', 'PART_TIME', 'PER_DIEM', 'CONTRACT', 'TEMPORARY', 'SEASONAL')`
    );
    table.check(
      `employment_status IN ('ACTIVE', 'ON_LEAVE', 'SUSPENDED', 'TERMINATED', 'RETIRED')`
    );
    table.check(
      `role IN ('CAREGIVER', 'SENIOR_CAREGIVER', 'CERTIFIED_NURSING_ASSISTANT', 'HOME_HEALTH_AIDE', 'PERSONAL_CARE_AIDE', 'COMPANION', 'NURSE_RN', 'NURSE_LPN', 'THERAPIST', 'COORDINATOR', 'SUPERVISOR', 'SCHEDULER', 'ADMINISTRATIVE')`
    );
    table.check(
      `status IN ('APPLICATION', 'INTERVIEWING', 'PENDING_ONBOARDING', 'ONBOARDING', 'ACTIVE', 'INACTIVE', 'ON_LEAVE', 'SUSPENDED', 'TERMINATED', 'RETIRED')`
    );
    table.check(
      `compliance_status IN ('COMPLIANT', 'PENDING_VERIFICATION', 'EXPIRING_SOON', 'EXPIRED', 'NON_COMPLIANT')`
    );
    table.check(
