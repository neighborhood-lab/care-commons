...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(
    "CREATE INDEX idx_invite_tokens_email ON invite_tokens(email) WHERE status = 'PENDING'"
  );
  ...
  await knex.raw(
    "COMMENT ON TABLE invite_tokens IS 'Email-based invitation tokens for team member registration'"
  );
  await knex.raw(
    "COMMENT ON COLUMN invite_tokens.token IS 'Unique cryptographically secure token for invitation link'"
  );
  await knex.raw(
    "COMMENT ON COLUMN invite_tokens.expires_at IS 'Token expiration timestamp (typically 7 days from creation)'"
  );
  await knex.raw(
    "COMMENT ON COLUMN invite_tokens.status IS 'Token lifecycle status: PENDING, ACCEPTED, EXPIRED, REVOKED'"
  );
}
