...
async function seedShiftMatching(pool: Pool): Promise<void> {
  ...
  const clientResult = await pool.query(
    'SELECT id FROM clients WHERE organization_id = $1 LIMIT 5',
    [organizationId]
  );
  ...
  const serviceTypeResult = await pool.query(
    'SELECT id FROM service_types WHERE organization_id = $1 LIMIT 1',
    [organizationId]
  );
...
async function createMatchingConfiguration(pool: Pool, context: SeedContext): Promise<string> {
...
async function createVisitsForMatching(pool: Pool, context: SeedContext): Promise<string[]> {
...
async function createProposalScenarios(
  pool: Pool,
  context: SeedContext,
  openShiftIds: string[]
): Promise<void> {
