...
const envSchema = z.object({
  ...
  (data) => {
    ...
    if (data.DATABASE_URL != null && data.DATABASE_URL.length > 0) {
      return true;
    }
    return data.DB_HOST != null && data.DB_HOST.length > 0 &&
           data.DB_NAME != null && data.DB_NAME.length > 0 &&
           data.DB_USER != null && data.DB_USER.length > 0 &&
           data.DB_PASSWORD != null && data.DB_PASSWORD.length > 0;
  },
  {
