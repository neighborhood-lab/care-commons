...
export class ShiftMatchingService {
   ...
   */
  async matchShift(input: MatchShiftInput, context: UserContext): Promise<MatchShiftResult> {
    // Get the open shift
    ...
    try {
      ...
      if (config === null) {
        throw new ValidationError(
          'No matching configuration found. Please create a default configuration first.'
        );
      }
   ...
   */
  async markProposalViewed(proposalId: UUID, context: UserContext): Promise<AssignmentProposal> {
    const proposal = await this.repository.getProposal(proposalId);
    if (proposal === null) {
      throw new NotFoundError('Proposal not found', { proposalId });
    }
  ...
  private async batchBuildCaregiverContexts(
    caregivers: Array<{
      id: UUID;
      primaryAddress?: { latitude?: number | null; longitude?: number | null };
    }>,
    shift: OpenShift,
    ...
    const caregiverMap = new Map(
      fullCaregivers.items.filter((cg) => caregiverIdSet.has(cg.id)).map((cg) => [cg.id, cg])
    );
    ...
    const rejectionsMap = new Map(
      rejectionsResult.rows.map((row: any) => [row.caregiver_id, parseInt(row.count ?? '0', 10)])
    );
    ...
    if (shift.latitude !== null && shift.longitude !== null) {
      ...
      for (const [id, caregiver] of caregiverMap) {
        if (
          caregiver.primaryAddress?.latitude !== null &&
  ...
  private async buildCaregiverContext(
    ...
    if (
      ...
      const distanceResult = await this.pool.query(
        ...
        [
          caregiver.primaryAddress.latitude,
          caregiver.primaryAddress.longitude,
          shift.latitude,
          shift.longitude,
        ]
      );
   ...
   */
  private async assignShift(visitId: UUID, caregiverId: UUID, context: UserContext): Promise<void> {
