...
exports.ShiftMatchingRepository = void 0;
const core_1 = require("@care-commons/core");
class ShiftMatchingRepository {
    constructor(pool) {
        this.pool = pool;
    }
    ...
    `;
        ...
        const existingCheck = await this.pool.query('SELECT id FROM open_shifts WHERE visit_id = $1', [input.visitId]);
        if (existingCheck.rows.length > 0) {
            throw new core_1.ConflictError('Open shift already exists for this visit', {
    ...
    `;
        ...
        if (result.rows.length === 0) {
            throw new core_1.NotFoundError('Open shift not found', { id });
        }
        return this.mapRowToOpenShift(result.rows[0]);
    }
    async searchOpenShifts(filters, pagination) {
        const conditions = [];
        const values = [];
        let paramCount = 0;
    ...
    async createMatchingConfiguration(input, context) {
        const query = `
    ...
    async getMatchingConfiguration(id) {
    ...
    async getDefaultConfiguration(organizationId, branchId) {
        const query = `
    ...
    async updateMatchingConfiguration(id, input, context) {
        const updates = [];
        const values = [];
        let paramCount = 0;
        ...
        if (updates.length === 0) {
            ...
            if (!existing) {
                throw new core_1.NotFoundError('Matching configuration not found', { id });
            }
            return existing;
        }
    ...
    async createProposal(input, matchScore, matchQuality, matchReasons, context) {
    ...
    async updateProposalStatus(id, status, context) {
        const query = `
    ...
    `;
        const result = await this.pool.query(query, [status, context.userId, id]);
        if (result.rows.length === 0) {
            throw new core_1.NotFoundError('Assignment proposal not found', { id });
        }
        return this.mapRowToProposal(result.rows[0]);
    }
    async respondToProposal(id, input, context) {
        const now = new Date();
        if (input.accept) {
            const query = `
        UPDATE assignment_proposals
    ...
    async getProposalsByCaregiver(caregiverId, statuses) {
        let query = `
    ...
    async getProposalsByOpenShift(openShiftId) {
        const query = `
      SELECT * FROM assignment_proposals
    ...
    async searchProposals(filters, pagination) {
    ...
    async getCaregiverPreferences(caregiverId) {
    ...
    async upsertCaregiverPreferences(caregiverId, organizationId, input, context) {
        const query = `
    ...
    async createBulkMatchRequest(input, context) {
        const query = `
    ...
    async updateBulkMatchRequest(id, updates, context) {
        const query = `
    ...
    `;
        ...
        const result = await this.pool.query(query, values);
        if (result.rows.length === 0) {
            throw new core_1.NotFoundError('Bulk match request not found', { id });
        }
        return this.mapRowToBulkMatchRequest(result.rows[0]);
    }
    async createMatchHistory(data, context) {
        const query = `
    ...
    mapRowToMatchingConfiguration(row) {
        return {
            id: row.id,
            organizationId: row.organization_id,
            branchId: row.branch_id,
...
exports.ShiftMatchingRepository = ShiftMatchingRepository;
//# sourceMappingURL=shift-matching-repository.js.map