...
export class ShiftMatchingRepository {
  ...
  private mapRowToOpenShift(row: Record<string, unknown>): OpenShift {
    const base = {
      ...
      taskIds: row.task_ids as string[] | undefined,
      requiredSkills: row.required_skills as string[] | undefined,
      requiredCertifications: row.required_certifications as string[] | undefined,
      preferredCaregivers: row.preferred_caregivers as string[] | undefined,
      blockedCaregivers: row.blocked_caregivers as string[] | undefined,
      genderPreference: row.gender_preference as 'MALE' | 'FEMALE' | 'NO_PREFERENCE' | undefined,
      languagePreference: row.language_preference as string | undefined,
      address: row.address as { line1: string; line2?: string; city: string; state: string; postalCode: string; country: string; accessInstructions?: string },
      latitude: row.latitude !== null && row.latitude !== undefined ? parseFloat(row.latitude as string) : undefined,
      longitude: row.longitude !== null && row.longitude !== undefined ? parseFloat(row.longitude as string) : undefined,
      priority: row.priority as 'LOW' | 'NORMAL' | 'HIGH' | 'CRITICAL',
      isUrgent: row.is_urgent as boolean,
      fillByDate: row.fill_by_date as Date | undefined,
      matchingStatus: row.matching_status as 'NEW' | 'MATCHING' | 'MATCHED' | 'PROPOSED' | 'ASSIGNED' | 'NO_MATCH' | 'EXPIRED',
      lastMatchedAt: row.last_matched_at as Date | undefined,
      matchAttempts: row.match_attempts as number,
      proposedAssignments: row.proposed_assignments as string[] | undefined,
      rejectedCaregivers: row.rejected_caregivers as string[] | undefined,
      clientInstructions: row.client_instructions as string | undefined,
  ...
  private mapRowToMatchingConfiguration(row: Record<string, unknown>): MatchingConfiguration {
    return {
      ...
      weights: row.weights as { skillMatch: number; availabilityMatch: number; proximityMatch: number; preferenceMatch: number; experienceMatch: number; reliabilityMatch: number; complianceMatch: number; capacityMatch: number },
      maxTravelDistance: row.max_travel_distance as number | undefined,
      maxTravelTime: row.max_travel_time as number | undefined,
      requireExactSkillMatch: row.require_exact_skill_match as boolean,
      requireActiveCertifications: row.require_active_certifications as boolean,
      respectGenderPreference: row.respect_gender_preference as boolean,
      respectLanguagePreference: row.respect_language_preference as boolean,
      autoAssignThreshold: row.auto_assign_threshold as number | undefined,
      minScoreForProposal: row.min_score_for_proposal as number,
      maxProposalsPerShift: row.max_proposals_per_shift as number,
      proposalExpirationMinutes: row.proposal_expiration_minutes as number,
      optimizeFor: row.optimize_for as 'BEST_MATCH' | 'FASTEST_FILL' | 'COST_EFFICIENT' | 'BALANCED_WORKLOAD' | 'CONTINUITY' | 'CAREGIVER_SATISFACTION',
      considerCostEfficiency: row.consider_cost_efficiency as boolean,
      balanceWorkloadAcrossCaregivers: row.balance_workload_across_caregivers as boolean,
      prioritizeContinuityOfCare: row.prioritize_continuity_of_care as boolean,
      preferSameCaregiverForRecurring: row.prefer_same_caregiver_for_recurring as boolean,
      penalizeFrequentRejections: row.penalize_frequent_rejections as boolean,
      boostReliablePerformers: row.boost_reliable_performers as boolean,
  ...
  private mapRowToProposal(row: Record<string, unknown>): AssignmentProposal {
    return {
      ...
      organizationId: row.organization_id as string,
      branchId: row.branch_id as string,
      openShiftId: row.open_shift_id as string,
      visitId: row.visit_id as string,
      caregiverId: row.caregiver_id as string,
      matchScore: row.match_score as number,
      matchQuality: row.match_quality as 'EXCELLENT' | 'GOOD' | 'FAIR' | 'POOR',
      matchReasons: row.match_reasons as Array<{category: 'SKILL' | 'EXPERIENCE' | 'PREFERENCE' | 'AVAILABILITY' | 'PROXIMITY' | 'RELIABILITY' | 'CLIENT_REQUEST' | 'SYSTEM_OPTIMIZED'; description: string; impact: 'POSITIVE' | 'NEGATIVE' | 'NEUTRAL'; weight: number}>,
      proposalStatus: row.proposal_status as 'PENDING' | 'SENT' | 'VIEWED' | 'ACCEPTED' | 'REJECTED' | 'EXPIRED' | 'SUPERSEDED' | 'WITHDRAWN',
      proposedBy: row.proposed_by as string,
      proposedAt: row.proposed_at as Date,
      proposalMethod: row.proposal_method as 'AUTOMATIC' | 'MANUAL' | 'CAREGIVER_SELF_SELECT',
      sentToCaregiver: row.sent_to_caregiver as boolean,
      sentAt: row.sent_at as Date | undefined,
      notificationMethod: row.notification_method as ('SMS' | 'EMAIL' | 'PUSH' | 'PHONE_CALL' | 'IN_APP') | undefined,
      viewedByCaregiver: row.viewed_by_caregiver as boolean,
      viewedAt: row.viewed_at as Date | undefined,
      respondedAt: row.responded_at as Date | undefined,
      responseMethod: row.response_method as ('MOBILE' | 'WEB' | 'PHONE' | 'IN_PERSON') | undefined,
      ...
      rejectionCategory: row.rejection_category as ('TOO_FAR' | 'TIME_CONFLICT' | 'PERSONAL_REASON' | 'PREFER_DIFFERENT_CLIENT' | 'RATE_TOO_LOW' | 'ALREADY_BOOKED' | 'NOT_INTERESTED' | 'OTHER') | undefined,
      expiredAt: row.expired_at as Date | undefined,
      isPreferred: row.is_preferred as boolean,
      urgencyFlag: row.urgency_flag as boolean,
      notes: row.notes as string | undefined,
      internalNotes: row.internal_notes as string | undefined,
      createdAt: row.created_at as Date,
      createdBy: row.created_by as string,
  ...
  private mapRowToPreferences(row: Record<string, unknown>): CaregiverPreferenceProfile {
    return {
      ...
      caregiverId: row.caregiver_id as string,
      organizationId: row.organization_id as string,
      preferredDaysOfWeek: row.preferred_days_of_week as string[] | undefined,
      preferredTimeRanges: row.preferred_time_ranges as Array<{startTime: string; endTime: string}> | undefined,
      preferredShiftTypes: row.preferred_shift_types as string[] | undefined,
      preferredClientIds: row.preferred_client_ids as string[] | undefined,
      preferredClientTypes: row.preferred_client_types as string[] | undefined,
      preferredServiceTypes: row.preferred_service_types as string[] | undefined,
      maxTravelDistance: row.max_travel_distance as number | undefined,
      preferredZipCodes: row.preferred_zip_codes as string[] | undefined,
      avoidZipCodes: row.avoid_zip_codes as string[] | undefined,
      maxShiftsPerDay: row.max_shifts_per_day as number | undefined,
      maxShiftsPerWeek: row.max_shifts_per_week as number | undefined,
      maxHoursPerWeek: row.max_hours_per_week as number | undefined,
      requireMinimumHoursBetweenShifts: row.require_minimum_hours_between_shifts as number | undefined,
      willingToAcceptUrgentShifts: row.willing_to_accept_urgent_shifts as boolean,
      willingToWorkWeekends: row.willing_to_work_weekends as boolean,
      willingToWorkHolidays: row.willing_to_work_holidays as boolean,
      acceptAutoAssignment: row.accept_auto_assignment as boolean,
      notificationMethods: row.notification_methods as Array<'SMS' | 'EMAIL' | 'PUSH' | 'PHONE_CALL' | 'IN_APP'>,
  ...
  private mapRowToBulkMatchRequest(row: Record<string, unknown>): BulkMatchRequest {
    return {
      ...
      organizationId: row.organization_id as string,
      branchId: row.branch_id as string | undefined,
      dateFrom: row.date_from as Date,
      dateTo: row.date_to as Date,
      openShiftIds: row.open_shift_ids as string[] | undefined,
      configurationId: row.configuration_id as string | undefined,
      optimizationGoal: row.optimization_goal as ('BEST_MATCH' | 'FASTEST_FILL' | 'COST_EFFICIENT' | 'BALANCED_WORKLOAD' | 'CONTINUITY' | 'CAREGIVER_SATISFACTION') | undefined,
      requestedBy: row.requested_by as string,
      requestedAt: row.requested_at as Date,
      status: row.status as 'PENDING' | 'IN_PROGRESS' | 'COMPLETED' | 'FAILED' | 'CANCELLED',
      startedAt: row.started_at as Date | undefined,
      completedAt: row.completed_at as Date | undefined,
      totalShifts: row.total_shifts as number,
      matchedShifts: row.matched_shifts as number,
      unmatchedShifts: row.unmatched_shifts as number,
      proposalsGenerated: row.proposals_generated as number,
  ...
  private mapRowToMatchHistory(row: Record<string, unknown>): MatchHistory {
    return {
      ...
      matchedAt: row.matched_at as Date,
      matchedBy: row.matched_by as string | undefined,
      matchScore: row.match_score as number | undefined,
      matchQuality: row.match_quality as ('EXCELLENT' | 'GOOD' | 'FAIR' | 'POOR') | undefined,
      outcome: row.outcome as 'PROPOSED' | 'ACCEPTED' | 'REJECTED' | 'EXPIRED' | 'SUPERSEDED' | 'NO_CANDIDATES' | 'MANUAL_OVERRIDE',
      outcomeDeterminedAt: row.outcome_determined_at as Date | undefined,
      assignmentProposalId: row.assignment_proposal_id as string | undefined,
      assignedSuccessfully: row.assigned_successfully as boolean,
      rejectionReason: row.rejection_reason as string | undefined,
      configurationId: row.configuration_id as string | undefined,
      configurationSnapshot: row.configuration_snapshot as Record<string, unknown> | undefined,
      responseTimeMinutes: row.response_time_minutes as number | undefined,
      notes: row.notes as string | undefined,
      createdAt: row.created_at as Date,
      createdBy: row.created_by as string,
      updatedAt: row.updated_at as Date,
