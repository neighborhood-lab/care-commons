...
class BillingRepository {
    ...
    async createPayer(payer, client) {
    ...
    async findPayerById(id) {
        const result = await this.pool.query('SELECT * FROM payers WHERE id = $1 AND deleted_at IS NULL', [id]);
        return result.rows[0] ? this.mapPayer(result.rows[0]) : null;
    }
    async findPayersByOrganization(organizationId) {
        const result = await this.pool.query(`SELECT * FROM payers 
       WHERE organization_id = $1 AND deleted_at IS NULL
       ORDER BY payer_name ASC`, [organizationId]);
        return result.rows.map(this.mapPayer);
    }
    async createRateSchedule(schedule, client) {
        const db = client || this.pool;
        const id = (0, uuid_1.v4)();
        const now = new Date();
        const result = await db.query(`
    ...
    async findActiveRateSchedule(organizationId, payerId, date = new Date()) {
        const query = payerId
    ...
    async createAuthorization(auth, client) {
        const db = client || this.pool;
        const id = (0, uuid_1.v4)();
        const now = new Date();
        const result = await db.query(`
    ...
    async findAuthorizationByNumber(authNumber) {
        const result = await this.pool.query('SELECT * FROM service_authorizations WHERE authorization_number = $1 AND deleted_at IS NULL', [authNumber]);
        return result.rows[0] ? this.mapAuthorization(result.rows[0]) : null;
    }
    async findActiveAuthorizationsForClient(clientId, serviceTypeId) {
    ...
    async updateAuthorizationUnits(authId, unitsUsed, unitsBilled, userId, client) {
        const db = client || this.pool;
        await db.query(`
    ...
    async createBillableItem(item, client) {
        const db = client || this.pool;
        const id = (0, uuid_1.v4)();
        const now = new Date();
        const result = await db.query(`
    ...
    async searchBillableItems(filters) {
    ...
    async updateBillableItemStatus(id, status, statusChange, userId, client) {
        const db = client || this.pool;
        await db.query(`
    ...
    async findInvoiceById(id) {
        const result = await this.pool.query('SELECT * FROM invoices WHERE id = $1 AND deleted_at IS NULL', [id]);
        return result.rows[0] ? this.mapInvoice(result.rows[0]) : null;
    }
    async findInvoiceByNumber(invoiceNumber) {
        const result = await this.pool.query('SELECT * FROM invoices WHERE invoice_number = $1 AND deleted_at IS NULL', [invoiceNumber]);
        return result.rows[0] ? this.mapInvoice(result.rows[0]) : null;
    }
    ...
    async updateInvoicePayment(id, paymentAmount, paymentReference, userId, client) {
        const db = client || this.pool;
        await db.query(`
      UPDATE invoices
      SET 
    ...
    async createPayment(payment, client) {
        const db = client || this.pool;
        const id = (0, uuid_1.v4)();
        const now = new Date();
        const result = await db.query(`
    ...
    async allocatePayment(paymentId, allocation, userId, client) {
        const db = client || this.pool;
        await db.query(`
      UPDATE payments
      SET 
