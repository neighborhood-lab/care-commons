...
export class PayrollRepository {
  ...
  private mapPayPeriod(row: Record<string, unknown>): PayPeriod {
    return {
      ...
      cutoffDate: row.cutoff_date as Date | undefined,
      approvalDeadline: row.approval_deadline as Date | undefined,
      payRunId: row.pay_run_id as UUID | undefined,
      totalCaregivers: row.total_caregivers as number | undefined,
      totalHours: row.total_hours as number | undefined,
      totalGrossPay: row.total_gross_pay as number | undefined,
      totalNetPay: row.total_net_pay as number | undefined,
      totalTaxWithheld: row.total_tax_withheld as number | undefined,
      totalDeductions: row.total_deductions as number | undefined,
      notes: row.notes as string | undefined,
      fiscalQuarter: row.fiscal_quarter as number | undefined,
      fiscalYear: row.fiscal_year as number | undefined,
  ...
  private mapTimeSheet(row: Record<string, unknown>): TimeSheet {
    return {
      ...
      submittedBy: row.submitted_by as UUID | undefined,
  ...
  private mapPayRun(row: Record<string, unknown>): PayRun {
    return {
      ...
      statusHistory: JSON.parse(row.status_history as string || '[]'),
      initiatedAt: row.initiated_at as Date | undefined,
      initiatedBy: row.initiated_by as UUID | undefined,
      calculatedAt: row.calculated_at as Date | undefined,
      approvedAt: row.approved_at as Date | undefined,
      approvedBy: row.approved_by as UUID | undefined,
      processedAt: row.processed_at as Date | undefined,
      processedBy: row.processed_by as UUID | undefined,
      ...
      payrollRegisterUrl: row.payroll_register_url as string | undefined,
      taxReportUrl: row.tax_report_url as string | undefined,
  ...
  private mapPayStub(row: Record<string, unknown>): PayStub {
    return {
      ...
      caregiverName: row.caregiver_name as string,
      caregiverEmployeeId: row.caregiver_employee_id as string,
      caregiverAddress: row.caregiver_address ? JSON.parse(row.caregiver_address as string) : undefined,
      payPeriodStartDate: row.pay_period_start_date as Date,
      payPeriodEndDate: row.pay_period_end_date as Date,
      payDate: row.pay_date as Date,
      ...
      paymentId: row.payment_id as UUID | undefined,
      bankAccountId: row.bank_account_id as UUID | undefined,
      bankAccountLast4: row.bank_account_last4 as string | undefined,
      checkNumber: row.check_number as string | undefined,
      checkDate: row.check_date as Date | undefined,
      checkStatus: row.check_status as PayStub['checkStatus'],
      status: row.status as PayStub['status'],
      statusHistory: JSON.parse(row.status_history as string || '[]'),
      calculatedAt: row.calculated_at as Date,
      calculatedBy: row.calculated_by as UUID | undefined,
      approvedAt: row.approved_at as Date | undefined,
      approvedBy: row.approved_by as UUID | undefined,
      deliveredAt: row.delivered_at as Date | undefined,
      deliveryMethod: row.delivery_method as PayStub['deliveryMethod'],
  ...
  private mapTaxConfiguration(row: Record<string, unknown>): TaxConfiguration {
    return {
      ...
      localTaxJurisdiction: row.local_tax_jurisdiction as string | undefined,
      ...
      w4FileDate: row.w4_file_date as Date | undefined,
      w4DocumentId: row.w4_document_id as UUID | undefined,
      stateFormOnFile: row.state_form_on_file as boolean,
      stateFormDate: row.state_form_date as Date | undefined,
      stateFormDocumentId: row.state_form_document_id as UUID | undefined,
      createdAt: row.created_at as Date,
      createdBy: row.created_by as UUID,
      updatedAt: row.updated_at as Date,
      updatedBy: row.updated_by as UUID,
  ...
  private mapDeduction(row: Record<string, unknown>): Deduction {
    return {
      ...
      yearlyLimit: row.yearly_limit as number | undefined,
      yearToDateAmount: row.year_to_date_amount as number | undefined,
      ...
      employerMatch: row.employer_match as number | undefined,
      employerMatchPercentage: row.employer_match_percentage as number | undefined,
      garnishmentOrder: row.garnishment_order ? JSON.parse(row.garnishment_order as string) : undefined,
  ...
  private mapPaymentRecord(row: Record<string, unknown>): PaymentRecord {
    return {
      ...
      accountType: row.account_type as 'CHECKING' | 'SAVINGS' | undefined,
      transactionId: row.transaction_id as string | undefined,
      traceNumber: row.trace_number as string | undefined,
      checkNumber: row.check_number as string | undefined,
      checkDate: row.check_date as Date | undefined,
      checkStatus: row.check_status as CheckStatus | undefined,
      checkClearedDate: row.check_cleared_date as Date | undefined,
      checkImageUrl: row.check_image_url as string | undefined,
      ...
      achBatchId: row.ach_batch_id as UUID | undefined,
      achFileId: row.ach_file_id as string | undefined,
      hasErrors: row.has_errors as boolean,
      errorCode: row.error_code as string | undefined,
      errorMessage: row.error_message as string | undefined,
      errorDetails: row.error_details as string | undefined,
      isReissue: row.is_reissue as boolean,
      originalPaymentId: row.original_payment_id as UUID | undefined,
      reissueReason: row.reissue_reason as string | undefined,
      notes: row.notes as string | undefined,
      createdAt: row.created_at as Date,
      createdBy: row.created_by as UUID,
      updatedAt: row.updated_at as Date,
  ...
  private mapACHBatch(row: Record<string, unknown>): ACHBatch {
    return {
      ...
      achFileUrl: row.ach_file_url as string | undefined,
      achFileFormat: row.ach_file_format as ACHBatch['achFileFormat'],
      achFileGeneratedAt: row.ach_file_generated_at as Date | undefined,
      achFileHash: row.ach_file_hash as string | undefined,
      ...
      settledAt: row.settled_at as Date | undefined,
      settlementConfirmation: row.settlement_confirmation as string | undefined,
      hasReturns: row.has_returns as boolean,
      returnCount: row.return_count as number | undefined,
