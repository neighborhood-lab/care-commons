...
export class AggregatorRouter {
   ...
   * - HHAeXchange serves TX (1 state) and FL (multi-aggregator option)
   *
  ...
  private getAggregator(state: StateCode): IAggregator {
    ...
    switch (aggregatorType) {
      ...
      case 'HHAEEXCHANGE':
        // HHAeXchange serves TX and is an option for FL
        return this.hhaeexchangeAggregator;

      case 'MULTI':
        // Florida multi-aggregator - for FL, route to specific aggregator based on config
        // In this implementation, we'll default to HHAeXchange for FL
        return this.hhaeexchangeAggregator;
