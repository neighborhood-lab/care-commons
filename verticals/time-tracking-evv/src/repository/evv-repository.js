...
exports.EVVRepository = void 0;
class EVVRepository {
    constructor(database) {
        this.database = database;
    }
    async createEVVRecord(record) {
        const query = `
    ...
    async searchEVVRecords(filters, pagination) {
        const conditions = [];
        const values = [];
        let paramIndex = 1;
    ...
    async getTimeEntriesByVisitId(visitId) {
        const query = `
    ...
    async getPendingTimeEntries(organizationId, limit = 100) {
        const query = `
      SELECT * FROM time_entries
    ...
    async updateTimeEntry(id, updates, updatedBy) {
        const fields = [];
        const values = [];
        let paramIndex = 1;
        Object.entries(updates).forEach(([key, value]) => {
    ...
    async updateTimeEntryStatus(id, status, verificationPassed, issues, updatedBy) {
        const query = `
      UPDATE time_entries
      SET status = $1,
    ...
    async getGeofenceByAddress(addressId) {
        const query = `
    ...
    async updateGeofenceStats(id, successful, accuracy) {
        const query = `
      UPDATE geofences
...
exports.EVVRepository = EVVRepository;
