...
class ScheduleRepository {
    ...
    async getServicePatternById(id) {
        const query = `
    ...
    async getPatternsByClient(clientId) {
        const query = `
      SELECT * FROM service_patterns
    ...
    async updateVisitStatus(id, newStatus, context, notes, reason) {
    ...
    async searchVisits(filters, pagination) {
    ...
    async getVisitsByDateRange(organizationId, startDate, endDate, branchIds) {
        let query = `
      SELECT * FROM visits
    ...
    async getUnassignedVisits(organizationId, branchId) {
        let query = `
      SELECT * FROM visits
      WHERE organization_id = $1
    ...
    async generateVisitNumber(organizationId) {
        const query = `
