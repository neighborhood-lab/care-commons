...
exports.CarePlanRepository = void 0;
const core_1 = require("@care-commons/core");
class CarePlanRepository extends core_1.Repository {
    ...
    async updateCarePlan(id, input, updatedBy) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
    ...
    async searchCarePlans(filters, pagination) {
    ...
    async getCarePlansByClientId(clientId) {
        const query = `
      SELECT * FROM care_plans
    ...
    async getActiveCarePlanForClient(clientId) {
        const query = `
      SELECT * FROM care_plans
      WHERE client_id = $1
        AND status = 'ACTIVE'
    ...
    async getExpiringCarePlans(organizationId, daysUntilExpiration) {
        const query = `
      SELECT * FROM care_plans
    ...
    async deleteCarePlan(id, deletedBy) {
        const query = `
      UPDATE care_plans
      SET deleted_at = NOW(),
          deleted_by = $1,
    ...
    async updateTaskInstance(id, updates, updatedBy) {
        const updateFields = [];
        const values = [];
        let paramIndex = 1;
    ...
    async searchTaskInstances(filters, pagination) {
        const conditions = [];
        const values = [];
        let paramIndex = 1;
    ...
    async getTasksByVisitId(visitId) {
        const query = `
      SELECT * FROM task_instances
      WHERE visit_id = $1
    ...
    async createProgressNote(input) {
        const query = `
    ...
    async getProgressNotesByCarePlanId(carePlanId) {
        const query = `
...
exports.CarePlanRepository = CarePlanRepository;
exports.default = CarePlanRepository;
//# sourceMappingURL=care-plan-repository.js.map