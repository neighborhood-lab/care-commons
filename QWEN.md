...
The following CLI tools are installed and available for responsible use:
...
2. **No production shortcuts**: Always follow branching strategy (`feature/*` → `develop` → `preview` → `main`)
...
6. **Pre-commit enforcement**: CLI tools cannot bypass pre-commit hooks
