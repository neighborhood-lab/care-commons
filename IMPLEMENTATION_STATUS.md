...
This document tracks the implementation status of Texas and Florida
state-specific requirements for the Scheduling & Visit Management and Time
Tracking & EVV verticals.
...
const clientData = await this.clientProvider.getClientForEVV(
  visitData.clientId
);
const caregiverData = await this.caregiverProvider.getCaregiverForEVV(
  input.caregiverId
);
