...
This document details the performance optimizations implemented to improve
database query performance across the Care Commons platform, with a focus on the
shift matching vertical.
...
The shift matching system performs complex queries filtering open shifts by
multiple criteria (organization, date range, status, priority, etc.). Without
proper composite indexes, these queries resulted in sequential scans or
...
- Dashboard queries loading active shifts: **60-75% reduction** in execution
  time
...
Multi-tenant queries filtering by `organization_id`, `status`, and `deleted_at`
were using multiple index scans or bitmap heap scans, resulting in poor
...
The shift matching algorithm checks for scheduling conflicts by querying visits
for each caregiver, looking for overlapping time ranges on the same date. This
was a major performance bottleneck.
...
Queries fetching proposals for caregivers or open shifts required sorting and
filtering that wasn't efficiently indexed.
...
Settings and state-specific JSONB fields are frequently queried using JSON path
operators (`->`, `->>`, `@>`, etc.). Without GIN indexes, these queries
...
The `createOpenShift` function performs a JOIN between `visits` and
`service_patterns` to fetch all required data. This required multiple heap
fetches.
...
The `matchShift` function was evaluating caregivers in a loop, making individual
database queries for each caregiver to fetch:
...
Implemented `batchBuildCaregiverContexts` that fetches all data in **7 batched
queries** regardless of caregiver count:
...
6. **Distances**: Single batch calculation using `unnest()` for coordinate
   arrays
...
- **Linear scalability**: Performance stays consistent regardless of caregiver
  count
